...
- `flow/` holds the design tree derived from `WW2_Naval_Roguelite_Game_Logic.md`: `FLOW.md` is the state machine and every branch out of each state, and `EVENTS.md` is the event grammar and content catalog. Both cite the design-document line behind every rule, and mark a branch `OPEN` rather than inventing content for it. Read `FLOW.md` before proposing a mechanic; its section 16 lists what has no content yet and section 17 lists where the repo contradicts itself.
