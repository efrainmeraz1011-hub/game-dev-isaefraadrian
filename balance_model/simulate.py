...
def config(name):
 ...
 if name=='candidate':
  c.update(enemy_growth=.16,enemy_dps_growth=.10,boss_hp=260.,boss_dps=.30,starting_cash=30.,reward_scale=.75)
 return c
...
def run(b,seed,c,policy='balanced',difficulty=1.,trace=False,scen=None):
 ...
 for s,nodes in enumerate(route):
  ...
  if not cause:
   ...
   reserve=15+21*c.get('boss_refit_count',1) if c.get('boss_refit') and s==5 and deck=='RRR' else 15
   while upgrades<8 and cash>=c['upgrade_cost']*(1+.18*upgrades)+reserve:
 ...
 if not cause:
  ...
  refit_count=c.get('boss_refit_count',1)
  if c.get('boss_refit') and deck=='RRR' and cash>=21*refit_count:
   cash-=21*refit_count;deck='RRH' if refit_count==1 else 'RHH';counters['boss_refit_paid']=21*refit_count
